...
add_action( 'wp_ajax_customer_related_orders', 'ajax__customer_related_orders' );
add_action( 'wp_ajax_nopriv_customer_related_orders', 'ajax__customer_related_orders' );
...
function customer_related_orders_meta_box_content( $post ) {
	...
	$orders = action_retrieve_customer_related_orders( true, $order->get_billing_email(), 0 );
		...
		$order_html .= '<table id="customer_related_orders" class="wp-list-table widefat fixed striped table-view-excerpt posts">';
			...
			$order_html .= '<thead>';
				...
				$order_html .= '<tr>';
					...
					$order_html .= '<th>' . esc_html__( 'View', 'customer-related-orders' ) . ' </th>';
					$order_html .= '<th>' . esc_html__( 'Order Number', 'customer-related-orders' ) . ' </th>';
			...
			$order_html .= render_customer_related_orders( $post->ID, $orders );
	...
	if ( $orders->max_num_pages > 0 ) {
		$order_html .= '<div class="order-pagination tablenav bottom">';
			...
			$order_html .= '<div class="alignright">';
				...
				$order_html .= '<select id="customer-related-orders-pagination" class="" data-email="' . esc_attr( $order->get_billing_email() ) . '" name="page">';
			...
			$order_html .= '  |  <span> ' . esc_html__( 'Orders', 'customer-related-orders' ) . ': ' . $orders->total . '</span>';
				} // phpcs:ignore
 ...
 */
function ajax__customer_related_orders() {
	...
	if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( $nonce, 'customer-related-orders' ) ) {
		if ( isset( $_POST['action'] ) && 'customer_related_orders' === $_POST['action'] && ! empty( $_POST['data'] ) && isset( $_POST['data']['email'] ) ) {
			$data = action_retrieve_customer_related_orders( true, $_POST['data']['email'], $_POST['data']['page'] ); // phpcs:ignore
			$data = render_customer_related_orders( get_queried_object_id(), $data );
			echo wp_kses(
				$data,
				array(
					'span'   => array(
 ...
 */
function action_retrieve_customer_related_orders( $allorders, $email, $offset ) {
 ...
 */
function render_customer_related_orders( $currentorder, $orders ) {
	$related_orders_html = '';
	foreach ( $orders->orders as $order ) {
		$related_orders_html     .= '<tr>';
			$related_orders_html .= '<td><a title="' . esc_html__( 'View order', 'customer-related-orders' ) . '" href=""><span class="dashicons dashicons-welcome-view-site"></span></a>';
